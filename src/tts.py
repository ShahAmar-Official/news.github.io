...
- Newscast-style voices are prioritised to give the channel an authoritative,
...
# ---------------------------------------------------------------------------
# Voice pool — newscast-style voices are listed first so they are selected
# most often.  The pool then continues with additional high-quality neural
# voices for variety.  24 voices total, rotating hourly.
# ---------------------------------------------------------------------------
_VOICE_POOL: list[dict] = [
    # --- Newscast-style voices (primary — listed first for priority rotation) ---
    ...
    {"name": "en-US-AriaNeural",         "gender": "female", "style": "newscast"},
    # --- Additional high-quality female voices ---
    ...
    # --- Additional high-quality male voices ---
...
def pick_voice() -> str:
    ...
    is scheduled hourly) uses a different voice character.  Newscast-style
    voices appear first in the pool so they are chosen during the first
    several hours of each day, giving the channel a consistently authoritative
    broadcast tone.
