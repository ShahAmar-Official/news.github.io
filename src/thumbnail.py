...
Creates a professional news-channel gradient background with large bold title
text, a BREAKING NEWS banner bar, rounded accent elements, and a topic-relevant
emoji as a visual accent.
"""
...
# Colour palette — news channel style
_BG_COLOR_TOP = (0, 20, 60)        # dark blue (news authority)
_BG_COLOR_BOTTOM = (180, 0, 0)     # deep red (urgency / breaking news)
_ACCENT_COLOR = (220, 20, 20)      # breaking-news red
_TEXT_COLOR = (255, 255, 255)       # white
_STROKE_COLOR = (0, 0, 0)          # black for outline
_BANNER_BG_COLOR = (220, 20, 20)   # red banner background
_BANNER_TEXT_COLOR = (255, 255, 255)  # white banner text
...
def _topic_emoji(topic: str) -> str:
    """Return a news-relevant emoji that loosely matches the topic keyword."""
    topic_lower = topic.lower()
    mapping = [
        (["breaking", "alert", "urgent", "developing"], "🚨"),
        (["election", "vote", "president", "congress", "parliament", "political"], "🗳️"),
        (["war", "conflict", "military", "troops", "nato", "ceasefire"], "⚔️"),
        (["economy", "market", "stock", "inflation", "gdp", "trade", "tariff"], "📊"),
        (["climate", "environment", "carbon", "emissions", "green", "renewable"], "🌍"),
        (["ai", "artificial intelligence", "tech", "robot", "cyber", "data"], "🤖"),
        (["space", "nasa", "rocket", "satellite", "planet", "mars"], "🚀"),
        (["health", "medical", "hospital", "vaccine", "disease", "pandemic"], "🏥"),
        (["science", "research", "discovery", "experiment", "study"], "🔬"),
        (["court", "justice", "supreme", "ruling", "law", "legal"], "⚖️"),
        (["crypto", "bitcoin", "blockchain", "digital currency"], "₿"),
        (["fire", "explosion", "disaster", "earthquake", "flood", "storm"], "⚠️"),
        (["diplomat", "summit", "treaty", "sanctions", "foreign"], "🌐"),
        (["energy", "oil", "gas", "nuclear", "power", "fuel"], "⚡"),
        (["crime", "arrest", "police", "fbi", "investigation", "fraud"], "🔍"),
        (["news", "press", "media", "reporter", "journalist"], "📰"),
    ]
...
def create_thumbnail(title: str, topic: str) -> Path:
    """Generate a 1280 × 720 JPEG news-channel thumbnail for the given video *title*.

    Args:
        title: The video title to display prominently.
        topic: The news story topic (used for emoji selection).
    ...
    # BREAKING NEWS / NEWS ALERT banner bar at the top
    banner_h = 80
    draw.rectangle([(0, 0), (THUMB_W, banner_h)], fill=_BANNER_BG_COLOR)

    banner_font = _load_font(44)
    banner_text = "⚡ BREAKING NEWS ⚡"
    try:
        bbox = draw.textbbox((0, 0), banner_text, font=banner_font)
        banner_text_w = bbox[2] - bbox[0]
        banner_x = (THUMB_W - banner_text_w) // 2
    except Exception:  # noqa: BLE001
        banner_x = 40
    _draw_text_with_stroke(
        draw, (banner_x, 16), banner_text,
        font=banner_font,
        fill=_BANNER_TEXT_COLOR,
        stroke_fill=(120, 0, 0),
        stroke_width=2,
    )
    ...
    glow_draw.ellipse(
        [(THUMB_W // 2 - 400, 80), (THUMB_W // 2 + 400, THUMB_H - 120)],
        fill=(15, 15, 40),
    )
    ...
    # Emoji (large, top-left region, below banner)
    emoji = _topic_emoji(topic)
    emoji_font = _load_font(120)
    try:
        draw.text((60, banner_h + 10), emoji, font=emoji_font, fill=_TEXT_COLOR)
    except Exception:  # noqa: BLE001
        pass
    ...
    start_y = max(banner_h + 140, (THUMB_H - total_text_h) // 2)
    ...
    _draw_text_with_stroke(
        draw, (70, bar_y + 18), "▶ NEWS SHORTS — SUBSCRIBE",
        font=watermark_font,
        fill=(255, 255, 255),
        stroke_fill=(180, 0, 0),
        stroke_width=0,
