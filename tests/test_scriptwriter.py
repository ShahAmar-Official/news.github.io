...
Tests the template-based news script generator with no external API calls.
...
class TestGenerateScript(unittest.TestCase):
    ...
    def test_returns_required_keys(self):
        ...
        result = self.generate_script("AI regulation developments")
    ...
    def test_title_is_non_empty_string(self):
        result = self.generate_script("Climate change policy updates")
    ...
    def test_script_is_non_empty_string(self):
        result = self.generate_script("Space mission updates")
    ...
    def test_script_has_no_ssml_tags(self):
        ...
        result = self.generate_script("Supreme Court rulings")
    ...
    def test_tags_is_list_of_strings(self):
        result = self.generate_script("Stock market analysis")
        tags = result["tags"]
        self.assertIsInstance(tags, list)
        self.assertTrue(all(isinstance(t, str) for t in tags),
                        "all tags must be strings")

    def test_tags_contain_news_terms(self):
        """Tags should include news-specific terms."""
        result = self.generate_script("Geopolitical tensions")
        tags = result["tags"]
        news_tags = {"breaking news", "news today", "daily news", "world news",
                     "news shorts", "news alert", "news", "headlines"}
        tag_set = set(tags)
        # At least some news tags should be present
        self.assertTrue(
            news_tags & tag_set,
            f"No news-specific tags found in {tag_set}"
        )

    def test_scenes_is_non_empty_list(self):
        result = self.generate_script("Cybersecurity threats")
        scenes = result["scenes"]
        self.assertIsInstance(scenes, list)
        self.assertGreater(len(scenes), 0, "scenes list must not be empty")

    def test_description_is_string(self):
        result = self.generate_script("Renewable energy milestones")
        self.assertIsInstance(result["description"], str)

    def test_hook_is_non_empty_string(self):
        result = self.generate_script("Healthcare breakthroughs")
    ...
    def test_deterministic_structure(self):
        ...
        r1 = self.generate_script("Global economic outlook")
        r2 = self.generate_script("Global economic outlook")
        self.assertEqual(set(r1.keys()), set(r2.keys()))

    def test_various_topics_do_not_raise(self):
        """Scriptwriter must handle a variety of news topics without crashing."""
        topics = [
            "Breaking: President signs new bill",
    ...
    def test_title_has_news_branding(self):
        """Title should use news-channel style formatting."""
        result = self.generate_script("breaking election results")
        title = result["title"]
        # News titles should contain news keywords (case-insensitive check)
        # from the title templates
        news_indicators = ["breaking", "news", "alert", "confirmed", "urgent",
                           "brief", "story", "report", "update", "latest",
                           "changed", "matters", "60-second"]
        self.assertTrue(
            any(ind in title.lower() for ind in news_indicators),
            f"Title '{title}' does not appear to use news-channel branding"
        )
...
class TestCaptionScript(unittest.TestCase):
    ...
    def test_caption_script_is_string(self):
        from src.scriptwriter import generate_script
        result = generate_script("Geopolitical crisis update")
    ...
    def test_caption_script_no_markup(self):
        ...
        from src.scriptwriter import generate_script
        result = generate_script("Climate change summit")
